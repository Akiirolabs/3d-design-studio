      ...
      <div className="space-y-6 p-5">
        ...
        <section className="space-y-3"><h3 className="flex items-center gap-2 text-sm font-semibold"><UserRound className="h-4 w-4 text-sky-400" />Account</h3>
          {!props.user ? <form onSubmit={submitAuth} className="space-y-3 rounded-xl bg-slate-950/60 p-4">
            ...
            <form onSubmit={async e => { e.preventDefault(); await props.onChangeUsername(username); setUsername(''); }} className="flex flex-col gap-2 min-[320px]:flex-row"><input aria-label="New username" className={field} required minLength={3} maxLength={32} placeholder={props.user.username} value={username} onChange={e => setUsername(e.target.value)} /><button disabled={props.busy} className={button}>Change username</button></form>
            <form onSubmit={async e => { e.preventDefault(); await props.onChangePassword(currentPassword, newPassword); setCurrentPassword(''); setNewPassword(''); }} className="grid gap-2 md:grid-cols-[1fr_1fr_auto]"><input aria-label="Current password" className={field} type="password" required placeholder="Current password" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} /><input aria-label="New password" className={field} type="password" minLength={5} required placeholder="New password" value={newPassword} onChange={e => setNewPassword(e.target.value)} /><button disabled={props.busy} className={button}>Change</button></form>
